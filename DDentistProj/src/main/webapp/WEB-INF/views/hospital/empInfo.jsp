...
<!-- 
CSRF Token
서버에서는 뷰 페이지를 발행할 때 랜덤으로 생성된 Token을 같이 준 뒤 사용자 세션에 저장해둡니다. 그리고 사용자가 서버에
작업을 요청할 때 페이지에 Hidden으로 숨어있는 Token 값이 같이 서버로 전송되는데, 서버에서는 이 Token 값이 세션에
저장된 값과 일치하는지 확인하여 해당 요청이 위조된게 아니라는 것을 확인합니다. 일치 여부를 확인한 Token은 바로 폐기하고
새로운 뷰 페이지를 발행할 때마다 새로 생성합니다. -->
<style>
.deleteBtn {
	text-align: center;
}
...
// 직원 목록
// needSetupForm => boolean type (true 또는 false)
...
$(function(){
	...
	$(document).on("click", ".trSelect", function(e){
		...
		$.ajax({
			...
			success: function(data){
				...
				if(empRtrmYmd != null){ //이미 퇴사한 직원이라면
					empRtrmYmd = empRtrmYmd.replace(/(\d{4})(\d{2})(\d{2})/g, '$1-$2-$3'); //yyyy-mm-dd형식으로 변환
					$("#empRtrmYmd").val(empRtrmYmd);
				} else {
					$("#empRtrmYmd").val(''); //퇴사자가 아니라면 빈 값으로 출력
				}
	...
	$(document).on("click", '.deleteBtn', function(e){
		e.stopPropagation(); //이벤트 버블링 막기 //부모의 이벤트를 막고 자식의 이벤트만 실행하도록 함
					...
					<div class="card-body">
						...
						<div class="card-body" style="height: 400px; overflow-y: auto;">
							<table class="table table-bordered text-center">
								<thead>
									<tr>
										<th style="width: 20%;"><i class="fa-sharp fa-regular fa-pen-to-square"></i></th>
						...
						<!-- 페이지블록 처리 -->
						<div class="card-footer clearfix" id="dataTable_paginate" style="background-color: white;"></div>
					</div>
				</div>
			</div>
