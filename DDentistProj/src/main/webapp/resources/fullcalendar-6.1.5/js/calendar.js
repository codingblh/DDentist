...
function filter(arg){	// 이름 검색, 의사 선택, 예약 현황 선택
	var resvData = arg.event.extendedProps;
	
	var show_emp = true;	// 의사 
	var show_state = true;	// 예약 현황
	var show_pt = true;		// 환자 이름
	...
	if (empNums && empNums.length > 0) {
	  if (empNums[0] == "all") {
	    ...
	    show_emp = empNums.indexOf(resvData.empNum) >= 0;
	  }
	}
	...
	if (states && states.length > 0) {
	  if (states[0] == "all") {
	    ...
	    show_state = states.indexOf(resvData.status) >= 0;
	  }
	}
	...
	if(ptInfo != ''){
		...
		if( ptInfo.indexOf("(") >= 0 ){
			...
			var show_pt_Num = resvData.ptNum.indexOf(ptNum) >= 0;
			var show_pt_Nm = resvData.ptNm.indexOf(ptNm) >= 0;
			var show_pt = show_pt_Num && show_pt_Nm;
		} else {
			ptNm = ptInfo.split("(")[0];	// 환자 이름
			show_pt = resvData.ptNm.indexOf(ptNm) >= 0;
		}
		
	}
	
	// 필터 적용시 해당하는 event 만 보여준다.
